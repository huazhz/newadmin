...
?>
					<!-- Modal -->
                            <div class="modal fade" id="fontAwsomeIconSelector" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="myModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                            ...
                                            <div class="pull-right" style="max-width:200px;margin-right:15px;">
							                    <div class="input-group">
								                    <span class="input-group-addon"><i class="fa fa-search fa-fw"></i></span>
								                    	<input type="text" id="icon-keyword-selector" class="form-control" placeholder="search.." onkeydown="searhc4icon($(this).val())">
							                  	</div>
                                            </div>
                                        ...
                                        {
                                        ?>
                                        <div class="icon-box-selector btn btn-default" onclick="selectIcon('&#x<?=$v?>;')">
                                        <i class="fa fa-<?=$k?>"></i><br>
                                        <?=$k?>
                                        </div>
                                        <?
                                        }
...
$(document).ready(function() {
	 ...
	 $('.icons-selector').focus(function(){
	 	$('#fontAwsomeIconSelector').modal('show');
	 	senderObj = $(this).attr('id');
	 });
	 
	 
});
function searhc4icon(keyword)
{
	if(keyword != '')
	{
...
{
	$('#'+senderObj).val(iconCode);
	$('#icon-keyword-selector').val('');
	$('.icon-box-selector').show();
	$('#fontAwsomeIconSelector').modal('hide');
}
</script>